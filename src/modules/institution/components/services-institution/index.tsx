...
export default function Services({servicesData, institution, setInstitution}: Props) {
  ...
  const [selectedKeys, setSelectedKeys] = useState(
    new Set(institution?.services ? institution.services.map(service => service.toString()) : [])
  );
  const list = useMemo(
    () =>
      servicesData?.filter(option => option?.name?.toLowerCase().includes(search.toLowerCase())),
    [servicesData, search]
  );
  useEffect(() => {
    ...
    <>
      ...
      >
        <div className="h-[300px] w-full flex flex-col gap-2">
          ...
          >
            <TableHeader
              className="border-none shadow-none rounded-sm"
